...
public class Gamers2021A implements Gamers2021Interface {
		...
		for (int i = 0; i < index - 1; i++) {
			if (Char1[i] != '?' && Count1[i] == MaxCount) {
				if (Char1[i + 1] == '?' && Count1[i + 2] == MaxCount && Count1[i + 1] == 1)
					return MaxCount + 1;
				else if (i < index - 3 && Char1[i + 1] == '?' && Char1[i + 3] == '?' && Count1[i + 2] == MaxCount - 1
						&& Count1[i + 4] == MaxCount && Count1[i + 1] == 1 && Count1[i + 3] == 1)
					return MaxCount + 1;
			}
		}
		return MaxCount;
